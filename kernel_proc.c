...
Pid_t sys_Exec(Task call, int argl, void* args)
{
  ...
  if(call != NULL) {
     ...
     newproc->ptcb = init_PTCB(call, argl, args, newproc);

     
     newproc->ptcb->tcb = spawn_thread(newproc, start_main_thread); 

    
     newproc->ptcb->tcb->ptcb = newproc->ptcb;

     wakeup(newproc->ptcb->tcb);

     newproc->ptcb->refcount=0; 
  }
